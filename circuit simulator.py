...
def main():
    ...
    #list to store individual batch size fault coverage percents
    #this list will containt 125 percentages, from A-E
    ...
    # list to store detection rate
    TV_detectPercent = []


    for x in range (25):
        
        #last batch contains all the remainders
        if x == 24:
             # read each test vector for each batch set
            for x in range (x*batch, 254):

                # Since the inputRead takes the inputs in the opposite order, we reverse it before giving it to the inputRead
                TV_rev = str(TV_E_list[x])
        ...
        #doing this 24 times for this TV
        TV_detectPercent.append(len(detect_list)/len(fault_list)*100)
